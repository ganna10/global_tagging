...
data.df <- tbl_df(read.csv("W126_O3_data.csv", header = TRUE))
percent <- data.df %>%
  dplyr::select(-Metric) %>%
...
title.base <- ": Percent Contributions to W126 O3 for Emissions and Meteorology for Year 2010"
...
plotting <- function (data.frame, region) {  
  ...
  file.name <- paste0(region, "_W126_Percent_Contributions.pdf")
