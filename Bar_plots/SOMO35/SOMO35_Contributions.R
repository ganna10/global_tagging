...
get.data <- function (month) {
  ...
  file.name <- paste0("/work/users/jco/tagging_global/Scripts/Tier2_Analysis/SOMO35/Data/HTAP_SOMO35_", month.text, "_assigned_to_Tier2_regions.csv")
...
metric.data <- zonal.mean %>%
  rowwise() %>%
  mutate(Source = get.source(Species, Region), Metric = "SOMO35") 
write.csv(metric.data, file = "SOMO35_O3_data.csv", row.names = FALSE, quote = FALSE)
...
plotting <- function (region, data.frame) {
  if (region == "SAS") {
    ...
    p <- p + ylab("SOMO35 (ppbv.days)") + ggtitle(plot.title)
    p <- p + theme_bw()
    ...
    file.name <- paste0(region, "_SOMO35_O3_Contributions.pdf")
