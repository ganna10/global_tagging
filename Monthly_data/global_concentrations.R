...
p <- ggplot()
p <- p + geom_raster(data = sums %>% arrange(Source), aes(x = lon, y = lat, fill = Mixing.Ratio), hjust = 1, vjust = 1)
p <- p + geom_path(data = world.map.correct, aes(x = long, y = lat, group = group), size = 0.7)
p <- p + facet_wrap(~ Source, ncol = 4, scales = "free")
p <- p + theme_tufte()
...
p <- p + theme(plot.title = element_text(face = "bold", size = 40, hjust = 0.5))

CairoPDF(file = "July_allocated_landscape.pdf", width = 21, height = 13)
print(p)
dev.off()
