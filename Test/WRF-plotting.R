...
get.data <- function (month) {
  ...
  file.name <- paste0("/work/users/jco/WRF-HTAP_Analysis/Mean/", month.text, "_assigned_to_regions.csv")
...
data.df <- data.df %>% mutate(O3 = O3 * 1000)  
...
plotting <- function (data.frame, region) { 
  ...
  file.name <- paste0(region, "_Mean_O3.pdf")
  plot.title <- paste0(" Mean O3 (ppbv) in ", region, " with Contributions from different Emission Sources")
  ...
  p <- ggplot(data = df, aes(x = Month, y = Zonal.Mean, fill = Source))
