...
data.df <- tbl_df(read.csv("95th_Percentile_O3_data.csv", header = TRUE))
percent <- data.df %>%
  dplyr::select(-Metric) %>%
...
title.base <- ": Percent Contributions to 95th Percentile O3 for Emissions and Meteorology for Year 2010"
...
plotting <- function (data.frame, region) {  
  ...
  file.name <- paste0(region, "_95th_percentile_Percent_Contributions.pdf")
