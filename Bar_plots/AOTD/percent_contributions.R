...
data.df <- tbl_df(read.csv("AOT40D_O3_data.csv", header = TRUE))
percent <- data.df %>%
  dplyr::select(-Metric) %>%
...
title.base <- ": Percent Contributions to AOT40-D O3 for Emissions and Meteorology for Year 2010"
...
plotting <- function (data.frame, region) {  
  ...
  file.name <- paste0(region, "_AOT40D_Percent_Contributions.pdf")
