...
data <- tbl_df(read.csv(file = "SOMO35_O3_data.csv"))
...
plotting <- function (region, data.frame) {
  ...
  p2 <- p2 + ggtitle("Monthly SOMO35 O3 (ppbv.days) of Transported NOx Sources from Source Regions to Tier 2 Receptor Regions")
  ...
  file.name <- paste0(region, "_SOMO35_O3_Yearly_Cycle_Total_plus_Contributions_plus_Country_Tier2.pdf")
