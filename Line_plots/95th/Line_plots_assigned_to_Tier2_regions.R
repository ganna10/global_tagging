...
data <- tbl_df(read.csv(file = "95th_Percentile_O3_data.csv"))
...
plotting <- function (region, data.frame) {
  ...
  p <- p + ylab("O3 95th Percentile (ppbv)")
  ...
  p2 <- p2 + ggtitle("Monthly 95th Percentile O3 (ppbv) of Transported NOx Sources from Source Regions to Tier 2 Receptor Regions")
  ...
  file.name <- paste0(region, "_95th_Percentile_O3_Yearly_Cycle_Total_plus_Contributions_plus_Country_Tier2.pdf")
