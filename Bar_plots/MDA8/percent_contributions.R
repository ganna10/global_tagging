...
data.df <- tbl_df(read.csv("MDA8_O3_data.csv", header = TRUE))
percent <- data.df %>%
  dplyr::select(-Metric) %>%
...
title.base <- ": Percent Contributions to MDA8 O3 for Emissions and Meteorology for Year 2010"
...
plotting <- function (data.frame, region) {  
  ...
  file.name <- paste0(region, "_MDA8_Percent_Contributions.pdf")
