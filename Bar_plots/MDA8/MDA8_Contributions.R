...
get.data <- function (month) {
  ...
  file.name <- paste0("/work/users/jco/tagging_global/Scripts/Tier2_Analysis/MDA8/Data/HTAP_NOx_Tagging_", month.text, "_assigned_to_Tier2_regions.csv")
...
metric.data <- zonal.mean %>%
  rowwise() %>%
  mutate(Source = get.source(Species, Region), Metric = "MDA8") 
write.csv(metric.data, file = "MDA8_O3_data.csv", row.names = FALSE, quote = FALSE)
...
plotting <- function (region, data.frame) {
  if (region == "SAS") {
    ...
    file.name <- paste0(region, "_MDA8_O3_Contributions.pdf")
