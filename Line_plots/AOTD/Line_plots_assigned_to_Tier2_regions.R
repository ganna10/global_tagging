...
data <- tbl_df(read.csv(file = "AOT40D_O3_data.csv"))
...
title.base <- ": Monthly AOT40-D for Emissions and Meteorology for Year 2010\nTotal Ozone AOT40D (ppbv.hrs) from NOx Sources in Tier 2 Receptor Regions"
...
plotting <- function (region, data.frame) {
  ...
  custom.breaks <- seq(0, y.max, by = 250)
  ...
  custom.breaks <- seq(0, y.max, by = 50)
  ...
  p2 <- p2 + theme_bw()
  p2 <- p2 + scale_y_continuous(breaks = custom.breaks, labels = every.nth(custom.breaks, 2, inverse = TRUE, percent = FALSE))
  ...
  p2 <- p2 + ggtitle("Monthly AOT40D O3 (ppbv.hrs) of Transported NOx Sources from Source Regions to Tier 2 Receptor Regions")
  ...
  file.name <- paste0(region, "_AOT40D_O3_Yearly_Cycle_Total_plus_Contributions_plus_Country_Tier2.pdf")
