...
get.data <- function (month) {
  ...
  file.name <- paste0("/work/users/jco/tagging_global/Scripts/Tier2_Analysis/AOTN/Data/HTAP_AOTN_", month.text, "_assigned_to_Tier2_regions.csv")
...
metric.data <- zonal.mean %>%
  rowwise() %>%
  mutate(Source = get.source(Species, Region), Metric = "AOT40-N") 
write.csv(metric.data, file = "AOT40N_O3_data.csv", row.names = FALSE, quote = FALSE)

title.base <- ": Monthly AOT40-N O3 for Emissions and Meteorology for Year 2010"
...
plotting <- function (region, data.frame) {
  if (region == "SAS") {
    ...
    custom.breaks <- seq(0, y.max, by = 125)
    ...
    p <- p + ylab("AOT40-N (ppbv.hrs)") + ggtitle(plot.title)
    p <- p + theme_bw()
    ...
    file.name <- paste0(region, "_AOT40N_O3_Contributions.pdf")
