...
data <- tbl_df(read.csv(file = "AOT40N_O3_data.csv"))
...
title.base <- ": Monthly AOT40-N for Emissions and Meteorology for Year 2010\nTotal Ozone AOT40N (ppbv.hrs) from NOx Sources in Tier 2 Receptor Regions"
...
plotting <- function (region, data.frame) {
  ...
  p2 <- p2 + ggtitle("Monthly AOT40N O3 (ppbv.hrs) of Transported NOx Sources from Source Regions to Tier 2 Receptor Regions")
  ...
  file.name <- paste0(region, "_AOT40N_O3_Yearly_Cycle_Total_plus_Contributions_plus_Country_Tier2.pdf")
