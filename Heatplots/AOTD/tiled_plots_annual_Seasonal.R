...
data <- tbl_df(read.csv("AOT40D_O3_data.csv", header = TRUE))
...
bin.data <- function (number) {
    ...
    if (rounded < 100) {
        ...
        value <- "1000-2000"
    } else if (rounded >= 2000 & rounded < 4000) {
        value <- "2000-4000"
    }
    return (value)
}
...
discrete$Cat <- factor(discrete$Cat, levels = c("<100", "100-250", "250-500", "500-1000", "1000-2000", "2000-4000"))
...
p <- p + ggtitle("Annual AOT40D O3 (ppbv.hrs) in each Tier 2 Receptor Region from each Source Region")
p <- p + scale_fill_manual(values = rev(discrete.colours), limits = rev(levels(factor(discrete$Cat))), name = "AOT40D (ppbv.hrs)")
p <- p + heatmap.plot.lines()

CairoPDF(file = "Annual_AOT40D_O3_Tiles.pdf", width = 16, height = 9)
print(p)
dev.off()
...
p2 <- p2 + ggtitle("Percent Contributions to Annual AOT40D O3 (ppbv) in each Tier 2 Receptor Region from each Source Region")
...
CairoPDF(file = "Annual_Percent_Contributions_AOT40D_O3_Tiles.pdf", width = 16, height = 9)
print(p2)
dev.off()
...
seasonal.bin.data <- function (number) {
    ...
    if (rounded < 100) {
        ...
        value <- "500-1000"
    } else if (rounded >= 1000 & rounded < 3500) {
        value <- "1000-3500"
    } else if (rounded >= 3000 & rounded <= 7000) {
        value <- "3500-7000"
    }
    return (value)
}
...
seasonal.data$Cat <- factor(seasonal.data$Cat, levels =  c("<100", "100-250", "250-500", "500-1000", "1000-3500", "3500-7000"))
...
seasonal.plot <- function (df, season) {
  ...
  plot.title <- paste0(season, " AOT40D O3 (ppbv.hrs) in each Tier 2 Receptor Region from each Source Region")

  discrete.colours <- c("<100" = "#0352cb", "100-250" = "#86b650", "250-500" = "#f7c56c", "500-1000" = "#cc6329", "1000-3500" = "#dc3522", "3500-7000" = "#6c254f")
  
  p1 <- ggplot(data = plot.df, aes(x = Source, y = Region, fill = Cat))
  p1 <- p1 + ggtitle(plot.title)
  p1 <- p1 + scale_fill_manual(values = rev(discrete.colours), limits = rev(levels(factor(plot.df$Cat))), name = "AOT40D (ppb.hrs)")
  p1 <- p1 + heatmap.plot.lines()
  
  file.name = paste0(season, "_AOT40D_O3_Tiles.pdf")
...
seasonal.contribution.plot <- function (df, season) {
  plot.df <- df %>% filter(Season == season)
  plot.title <- paste0(season, " Pecent Contributions to AOT40D O3 (ppbv) in each Tier 2 Receptor Region from each Source Region")
  ...
  file.name = paste0(season, "_Percent_Contribution_AOT40D_O3_Tiles.pdf")
  CairoPDF(file = file.name, width = 16, height = 9)
  print(p1)
  dev.off()
}
