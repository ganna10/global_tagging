...
data.df <- tbl_df(read.csv("Mean_O3_data.csv", header = TRUE))
percent <- data.df %>%
  dplyr::select(-Metric) %>%
...
title.base <- ": Percent Contributions to Mean O3 for Emissions and Meteorology for Year 2010"
...
plotting <- function (data.frame, region) {  
  ...
  file.name <- paste0(region, "_Mean_Percent_Contributions.pdf")
