...
data <- tbl_df(read.csv("W126_O3_data.csv", header = TRUE))
...
bin.data <- function (number) {
    ...
    if (rounded < 0.1) {
        value <- "<0.1"
    } else if (rounded >= 0.1 & rounded < 0.5) {
        value <- "0.1-0.5"
    } else if (rounded >= 0.5 & rounded < 1) {
        value <- "0.5-1"
    } else if (rounded >= 1 & rounded < 2) {
        value <- "1-2"
    } else if (rounded >= 2 & rounded < 3) {
        value <- "2-3"
    } else if (rounded >= 3 & rounded < 4) {
        value <- "3-4"
    }
    return (value)
}
...
discrete$Cat <- factor(discrete$Cat, levels = c("<0.1", "0.1-0.5", "0.5-1", "1-2", "2-3", "3-4"))
...
p <- p + ggtitle("Annual W126 O3 (ppbv.days) in each Tier 2 Receptor Region from each Source Region")
p <- p + scale_fill_manual(values = rev(discrete.colours), limits = rev(levels(factor(discrete$Cat))), name = "W126 (ppmv.hrs)")
p <- p + heatmap.plot.lines()

CairoPDF(file = "Annual_W126_O3_Tiles.pdf", width = 16, height = 9)
print(p)
dev.off()
...
p2 <- p2 + ggtitle("Percent Contributions to Annual W126 O3 (ppbv) in each Tier 2 Receptor Region from each Source Region")
...
CairoPDF(file = "Annual_Percent_Contributions_W126_O3_Tiles.pdf", width = 16, height = 9)
print(p2)
dev.off()
...
seasonal.bin.data <- function (number) {
    ...
    if (rounded < 0.01) {
        value <- "<0.01"
    } else if (rounded >= 0.01 & rounded < 0.25) {
        value <- "0.01-0.25"
    } else if (rounded >= 0.25 & rounded < 0.5) {
        value <- "0.25-0.5"
    } else if (rounded >= 0.5 & rounded < 1) {
        value <- "0.5-1"
    } else if (rounded >= 1 & rounded < 5) {
        value <- "1-5"
    } else if (rounded >= 5 & rounded <= 10) {
        value <- "5-10"
    }
...
seasonal.data$Cat <- factor(seasonal.data$Cat, levels =  c("<0.01", "0.01-0.25", "0.25-0.5", "0.5-1", "1-5", "5-10"))
...
seasonal.plot <- function (df, season) {
  ...
  plot.title <- paste0(season, " W126 O3 (ppbv.days) in each Tier 2 Receptor Region from each Source Region")

  discrete.colours <- c("<0.01" = "#0352cb", "0.01-0.25" = "#86b650", "0.25-0.5" = "#f7c56c", "0.5-1" = "#cc6329", "1-5" = "#dc3522", "5-10" = "#6c254f")
  
  p1 <- ggplot(data = plot.df, aes(x = Source, y = Region, fill = Cat))
  p1 <- p1 + ggtitle(plot.title)
  p1 <- p1 + scale_fill_manual(values = rev(discrete.colours), limits = rev(levels(factor(plot.df$Cat))), name = "W126 (ppm.hrs)")
  p1 <- p1 + heatmap.plot.lines()
  
  file.name = paste0(season, "_W126_O3_Tiles.pdf")
...
seasonal.contribution.plot <- function (df, season) {
  plot.df <- df %>% filter(Season == season)
  plot.title <- paste0(season, " Pecent Contributions to W126 O3 (ppbv) in each Tier 2 Receptor Region from each Source Region")
  ...
  file.name = paste0(season, "_Percent_Contribution_W126_O3_Tiles.pdf")
  CairoPDF(file = file.name, width = 16, height = 9)
  print(p1)
  dev.off()
}
