...
get.data <- function (metric) {
  file.name <- paste0("/work/users/jco/tagging_global/Scripts/Tier2_Analysis/", metric, "/Data/HTAP_NOx_Tagging_2010_assigned_to_Tier2_regions.csv")
  data <- read.csv(file = file.name, header = TRUE)
  data$Metric <- rep(metric, nrow(data))
  
  if (metric == "Mean") {
    data$Mixing.Ratio <- data$Mixing.Ratio * 1e9
  }
  return(data)
}
...
zonal.mean <- data.df %>%
  mutate(Mixing.Ratio = Mixing.Ratio) %>%
  group_by(Species, Month, Region, Metric) %>%
...
zonal.mean$Metric <- factor(zonal.mean$Metric, levels = c("MDA8", "Mean"))
...
plotting <- function (region, data.frame) {
  if (region == "SAS") {
    ...
    plot.title <- "South Asia: Monthly O3 for Emissions and Meteorology for Year 2010"  
  } else if (region == "NAM") {
    df <- data.frame %>%
    ...
    plot.title <- "North America: Monthly O3 for Emissions and Meteorology for Year 2010"
  } else if (region == "EUR") {
    df <- data.frame %>%
    ...
    plot.title <- "Europe: Monthly O3 for Emissions and Meteorology for Year 2010"
  } else if (region == "EAS") {
    df <- data.frame %>%
    ...
    plot.title <- "East Asia: Monthly O3 for Emissions and Meteorology for Year 2010"
  } else if (region == "MDE") {
    df <- data.frame %>%
    ...
    plot.title <- "Middle East: Monthly O3 for Emissions and Meteorology for Year 2010"
  } else if (region == "RBU") {
    df <- data.frame %>%
    ...
    plot.title <- "Russia, Belarus, Ukraine: Monthly O3 for Emissions and Meteorology for Year 2010"
  } else if (region == "OCN") {
    df <- data.frame %>%
    ...
    plot.title <- "Oceans: Monthly O3 for Emissions and Meteorology for Year 2010"
  } else if (region == "Rest") {
    df <- data.frame %>%
      filter(Region == "Rest")
    ...
    plot.title <- "Rest of the World: Monthly O3 for Emissions and Meteorology for Year 2010"
  } else {
    stop("No region")
  }
  ...
  metric.colours <- c("MDA8" = "#6c254f", "Mean" = "#2b9eb3")
  
  p <- ggplot(data = with.sources, aes(x = Month, y = Zonal.Mean, colour = Metric, group = Metric))
  p <- p + geom_point()
  p <- p + geom_path()
  p <- p + facet_grid(Source ~ Region)
  p <- p + plot_theme()
  p <- p + ylab("O3 (ppbv)")
  p <- p + theme(axis.title = element_blank())
  p <- p + ggtitle(plot.title, subtitle = "\nOzone (ppbv) from NOx Sources in Tier 2 Receptor Regions")
  p <- p + scale_colour_manual(values = metric.colours, limits = levels(factor(with.sources$Metric)))
...
#     gather(Emission.Source, Mean, -Month, -Region, -Total, -Metric) %>%
#     mutate(Contribution = Mean / Total) %>%
#     dplyr::select(Month, Region, Emission.Source, Contribution, Mean, Metric)
# 
...
#   
#   p2 <- ggplot(data = transport, aes(x = Month, y = Mean, colour = Metric, group = Metric))
#   p2 <- p2 + geom_point()
#   p2 <- p2 + geom_line()
#   p2 <- p2 + facet_grid(Emission.Source ~ Region)
#   p2 <- p2 + plot_theme()
...
#   p2 <- p2 + ggtitle("Monthly MDA8 - Mean O3 (ppbv) of Transported NOx Sources from Source Regions to Tier 2 Receptor Regions")
...
#   p5 <- p5 + theme(plot.title = element_text(size = 12))
  ...
  file.name <- paste0(region, "_Metrics_O3_Yearly_Cycle_Total_plus_Contributions_plus_Country_Tier2.pdf")
