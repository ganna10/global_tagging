...
data <- tbl_df(read.csv("SOMO35_O3_data.csv", header = TRUE))
...
bin.data <- function (number) {
    ...
    if (rounded < 10) {
        ...
        value <- "25-50"
    } else if (rounded >= 50 & rounded < 100) {
        value <- "50-100"
...
discrete$Cat <- factor(discrete$Cat, levels = c("<10", "10-25", "25-50", "50-100", "100-250", "250-500"))
...
p <- p + ggtitle("Annual SOMO35 O3 (ppbv.days) in each Tier 2 Receptor Region from each Source Region")
p <- p + scale_fill_manual(values = rev(discrete.colours), limits = rev(levels(factor(discrete$Cat))), name = "SOMO35 (ppbv.days)")
p <- p + heatmap.plot.lines()

CairoPDF(file = "Annual_SOMO35_O3_Tiles.pdf", width = 16, height = 9)
print(p)
dev.off()
...
p2 <- p2 + ggtitle("Percent Contributions to Annual SOMO35 O3 (ppbv) in each Tier 2 Receptor Region from each Source Region")
...
CairoPDF(file = "Annual_Percent_Contributions_SOMO35_O3_Tiles.pdf", width = 16, height = 9)
print(p2)
dev.off()
...
seasonal.bin.data <- function (number) {
    ...
    if (rounded < 10) {
        ...
        value <- "10-20"
    } else if (rounded >= 20 & rounded < 50) {
        value <- "20-50"
    } else if (rounded >= 50 & rounded < 150) {
        value <- "50-150"
    } else if (rounded >= 150 & rounded < 500) {
        value <- "150-500"
    } else if (rounded >= 500 & rounded <= 900) {
        value <- "500-900"
    }
    return (value)
}
...
seasonal.data$Cat <- factor(seasonal.data$Cat, levels =  c("<10", "10-20", "20-50", "50-150", "150-500", "500-900"))
...
seasonal.plot <- function (df, season) {
  ...
  plot.title <- paste0(season, " SOMO35 O3 (ppbv.days) in each Tier 2 Receptor Region from each Source Region")

  discrete.colours <- c("<10" = "#0352cb", "10-20" = "#86b650", "20-50" = "#f7c56c", "50-150" = "#cc6329", "150-500" = "#dc3522", "500-900" = "#6c254f")
  
  p1 <- ggplot(data = plot.df, aes(x = Source, y = Region, fill = Cat))
  p1 <- p1 + ggtitle(plot.title)
  p1 <- p1 + scale_fill_manual(values = rev(discrete.colours), limits = rev(levels(factor(plot.df$Cat))), name = "SOMO35 (ppb.days)")
  p1 <- p1 + heatmap.plot.lines()
  
  file.name = paste0(season, "_SOMO35_O3_Tiles.pdf")
...
seasonal.contributions <- seasonal.data %>%
  ...
  mutate(Percent.Contribution = ifelse(Total == 0, 0, Zonal.Mean * 100 / Total)) %>%
...
seasonal.contribution.plot <- function (df, season) {
  plot.df <- df %>% filter(Season == season)
  plot.title <- paste0(season, " Pecent Contributions to SOMO35 O3 (ppbv) in each Tier 2 Receptor Region from each Source Region")
  ...
  file.name = paste0(season, "_Percent_Contribution_SOMO35_O3_Tiles.pdf")
  CairoPDF(file = file.name, width = 16, height = 9)
  print(p1)
  dev.off()
}
