...
data <- tbl_df(read.csv(file = "W126_O3_data.csv"))
...
plotting <- function (region, data.frame) {
  ...
  transported <- trans %>%
    ...
    ungroup %>%
    mutate(Transported = rowSums(.[3:9])) %>%
  ...
  custom.breaks.1 <- seq(0, y.max, by = 0.4)
  custom.breaks <- unlist(lapply(custom.breaks.1, function (x) { round(x, digits = 2) })) 
  ...
  p <- p + ylab("O3 W126 (ppmv.hrs)")
  p <- p + ggtitle(plot.title)
  p <- p + scale_y_continuous(breaks = custom.breaks, labels = every.nth(custom.breaks, 2, inverse = TRUE, percent = FALSE, decimals = 1))
  p <- p + theme_bw()
  ...
  y.max <- max(trans$Zonal.Mean)
  custom.breaks.1 <- seq(0, y.max, by = 0.1)
  custom.breaks <- unlist(lapply(custom.breaks.1, function (x) { round(x, digits = 2) }))
  ...
  p2 <- p2 + theme_bw()
  p2 <- p2 + scale_y_continuous(expand = c(0, 0), breaks = custom.breaks, labels = every.nth(custom.breaks, 2, inverse = TRUE, percent = FALSE, decimals = 1), limits = c(0, y.max))
  ...
  p2 <- p2 + ggtitle("Monthly W126 O3 (ppmv.hrs) of Transported NOx Sources from Source Regions to Tier 2 Receptor Regions")
  ...
  file.name <- paste0(region, "_W126_O3_Yearly_Cycle_Total_plus_Contributions_plus_Country_Tier2.pdf")
