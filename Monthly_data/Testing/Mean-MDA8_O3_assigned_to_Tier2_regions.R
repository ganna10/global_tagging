...
# MDA- Mean calculation
diff <- zonal.mean %>%
  spread(Metric, Zonal.Mean) %>%
  mutate(Difference = MDA8 - Mean) %>%
  dplyr::select(Species, Month, Region, Difference)
...
plotting <- function (region, data.frame) {
  if (region == "SAS") {
    ...
    plot.title <- "South Asia: Monthly MDA8 - Mean O3 for Emissions and Meteorology for Year 2010"  
  } else if (region == "NAM") {
    df <- data.frame %>%
    ...
    plot.title <- "North America: Monthly MDA8 - Mean O3 for Emissions and Meteorology for Year 2010"
  } else if (region == "EUR") {
    df <- data.frame %>%
    ...
    plot.title <- "Europe: Monthly MDA8 - Mean O3 for Emissions and Meteorology for Year 2010"
  } else if (region == "EAS") {
    df <- data.frame %>%
    ...
    plot.title <- "East Asia: Monthly MDA8 - Mean O3 for Emissions and Meteorology for Year 2010"
  } else if (region == "MDE") {
    df <- data.frame %>%
    ...
    plot.title <- "Middle East: Monthly MDA8 - Mean O3 for Emissions and Meteorology for Year 2010"
  } else if (region == "RBU") {
    df <- data.frame %>%
    ...
    plot.title <- "Russia, Belarus, Ukraine: Monthly MDA8 - Mean O3 for Emissions and Meteorology for Year 2010"
  } else if (region == "OCN") {
    df <- data.frame %>%
    ...
    plot.title <- "Oceans: Monthly MDA8 - Mean O3 for Emissions and Meteorology for Year 2010"
  } else if (region == "Rest") {
    df <- data.frame %>%
      filter(Region == "Rest")
    ...
    plot.title <- "Rest of the World: Monthly MDA8 - Mean O3 for Emissions and Meteorology for Year 2010"
  } else {
    stop("No region")
  }
  ...
  p <- ggplot(data = with.sources, aes(x = Month, y = Difference, colour = Source, group = Source))
  p <- p + geom_point()
  p <- p + geom_path()
  ...
  p <- p + ggtitle(plot.title, subtitle = "\nTotal Ozone MDA8 - Mean (ppbv) and Mean from NOx Sources in Tier 2 Receptor Regions")
...
#   
  transport <- df %>%
    ...
    group_by(Month, Region, Source) %>%
    summarise(Mixing.Ratio = sum(Difference)) %>%
...
#   p5 <- p5 + theme(plot.title = element_text(size = 12))
  ...
  file.name <- paste0(region, "_MDA8-Mean_O3_Yearly_Cycle_Total_plus_Contributions_plus_Country_Tier2.pdf")
...
lapply(emission.regions, plotting, data.frame = diff)
