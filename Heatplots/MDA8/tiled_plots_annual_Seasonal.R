...
data <- tbl_df(read.csv("MDA8_O3_data.csv", header = TRUE))
...
discrete$Cat <- factor(discrete$Cat, levels = c("<1", "1-2", "2-5", "5-10", "10-20", "20-40"))
...
p <- p + ggtitle("Annual MDA8 O3 (ppbv) in each Tier 2 Receptor Region from each Source Region")
...
CairoPDF(file = "Annual_MDA8_O3_Tiles.pdf", width = 16, height = 9)
print(p)
dev.off()
...
annual.contributions$Cat <- factor(annual.contributions$Cat, levels = c("<5", "5-10", "10-20", "20-30", "30-50", "50-75"))
...
p2 <- p2 + ggtitle("Percent Contributions to Annual MDA8 O3 (ppbv) in each Tier 2 Receptor Region from each Source Region")
...
CairoPDF(file = "Annual_Percent_Contributions_MDA8_O3_Tiles.pdf", width = 16, height = 9)
print(p2)
dev.off()
...
seasonal.data$Cat <- factor(seasonal.data$Cat, levels =  c("<2", "2-5", "5-10", "10-20", "20-40", "40-60"))
...
seasonal.plot <- function (df, season) {
  ...
  plot.title <- paste0(season, " MDA8 O3 (ppbv) in each Tier 2 Receptor Region from each Source Region")
  ...
  file.name = paste0(season, "_MDA8_O3_Tiles.pdf")
...
seasonal.contribution.plot <- function (df, season) {
  plot.df <- df %>% filter(Season == season)
  plot.title <- paste0(season, " Pecent Contributions to MDA8 O3 (ppbv) in each Tier 2 Receptor Region from each Source Region")
  ...
  file.name = paste0(season, "_Percent_Contribution_MDA8_O3_Tiles.pdf")
  CairoPDF(file = file.name, width = 16, height = 9)
  print(p1)
  dev.off()
}
