...
data <- tbl_df(read.csv("95th_Percentile_O3_data.csv", header = TRUE))
...
discrete$Cat <- factor(discrete$Cat, levels = c("<2", "2-5", "5-10", "10-20", "20-30", "30-55"))
...
p <- p + ggtitle("Annual 95th Percentile O3 (ppbv) in each Tier 2 Receptor Region from each Source Region")
...
CairoPDF(file = "Annual_95th_Percentile_O3_Tiles.pdf", width = 16, height = 9)
print(p)
dev.off()
...
p2 <- p2 + ggtitle("Percent Contributions to Annual 95th Percentile O3 (ppbv) in each Tier 2 Receptor Region from each Source Region")
...
CairoPDF(file = "Annual_Percent_Contributions_95th_Percentile_O3_Tiles.pdf", width = 16, height = 9)
print(p2)
dev.off()
...
seasonal.data$Cat <- factor(seasonal.data$Cat, levels =  c("<5", "5-10", "10-25", "25-50", "50-70", "70-85")) 
...
seasonal.plot <- function (df, season) {
  ...
  plot.title <- paste0(season, " 95th Percentile O3 (ppbv) in each Tier 2 Receptor Region from each Source Region")

  discrete.colours <- c("<5" = "#0352cb", "5-10" = "#86b650", "10-25" = "#f7c56c", "25-50" = "#cc6329", "50-70" = "#dc3522", "70-85" = "#6c254f")
  ...
  file.name = paste0(season, "_95th_Percentile_O3_Tiles.pdf")
...
seasonal.contribution.plot <- function (df, season) {
  plot.df <- df %>% filter(Season == season)
  plot.title <- paste0(season, " Pecent Contributions to 95th Percentile O3 (ppbv) in each Tier 2 Receptor Region from each Source Region")
  ...
  file.name = paste0(season, "_Percent_Contribution_95th_Percentile_O3_Tiles.pdf")
  CairoPDF(file = file.name, width = 16, height = 9)
  print(p1)
  dev.off()
}
