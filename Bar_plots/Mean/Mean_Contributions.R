...
metric.data <- zonal.mean %>%
  rowwise() %>%
  mutate(Source = get.source(Species, Region), Metric = "Mean") 
write.csv(metric.data, file = "Mean_O3_data.csv", row.names = FALSE, quote = FALSE)
...
plotting <- function (region, data.frame) {
  if (region == "SAS") {
    ...
    file.name <- paste0(region, "_Mean_O3_Contributions.pdf")
