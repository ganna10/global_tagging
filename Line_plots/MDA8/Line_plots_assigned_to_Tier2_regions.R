...
data <- tbl_df(read.csv(file = "MDA8_O3_data.csv"))
...
title.base <- ": Monthly MDA8 for Emissions and Meteorology for Year 2010\nTotal Ozone MDA8 (ppbv) from NOx Sources in Tier 2 Receptor Regions"
...
plotting <- function (region, data.frame) {
  ...
  p2 <- p2 + ggtitle("Monthly MDA8 O3 (ppbv) of Transported NOx Sources from Source Regions to Tier 2 Receptor Regions")
